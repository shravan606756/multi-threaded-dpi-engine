...
bool FPManager::queuesEmpty() const {
    for (const auto& fp : fps_) {
...
std::string FPManager::generateClassificationReport() const {
    ...
    std::ostringstream ss;
    const size_t total = total_classified + total_unknown;
    const double classified_pct = total > 0 ? (100.0 * total_classified / total) : 0.0;
    const double unknown_pct = total > 0 ? (100.0 * total_unknown / total) : 0.0;

    ss << "\n[Classification] Summary\n";
    ss << "  total_connections=" << total << "\n";
    ss << "  classified=" << total_classified << " (" << std::fixed << std::setprecision(1)
       << classified_pct << "%)\n";
    ss << "  unknown=" << total_unknown << " (" << std::fixed << std::setprecision(1)
       << unknown_pct << "%)\n";
    ...
    ss << "[Classification] Application distribution\n";
    ...
    if (!domain_counts.empty()) {
        std::vector<std::pair<std::string, size_t>> sorted_domains(domain_counts.begin(), domain_counts.end());
        std::sort(sorted_domains.begin(), sorted_domains.end(),
                  [](const auto& a, const auto& b) { return a.second > b.second; });

        const size_t top_n = std::min<size_t>(10, sorted_domains.size());
        ss << "[Classification] Top domains\n";
        for (size_t i = 0; i < top_n; ++i) {
            ss << "  - " << sorted_domains[i].first << ": " << sorted_domains[i].second << "\n";
        }
    }
