...
DPIEngine::DPIEngine(const Config& config)
    ...
    const int total_fps = config.num_load_balancers * config.fps_per_lb;
    std::cout << "\n[DPIEngine] Starting DPI engine v1.0\n";
    std::cout << "[DPIEngine] Config: load_balancers=" << config.num_load_balancers
              << " fps_per_lb=" << config.fps_per_lb
              << " total_fp_threads=" << total_fps << "\n";
}
...
bool DPIEngine::initialize() {
    ...
    fp_manager_ = std::make_unique<FPManager>(total_fps, rule_manager_.get(), output_cb);
        ...
        auto fp_queue_ptrs = fp_manager_->getQueuePtrs();
    std::cerr << "[DPIEngine][DEBUG] FP queue pointers: " << fp_queue_ptrs.size()
              << " expected=" << total_fps << "\n";
...
bool DPIEngine::processFile(const std::string& input_file,
    ...
    std::cout << "[DPIEngine] Waiting for pipeline drain...\n";
    while (true) {
        const bool lbs_empty = !lb_manager_ || lb_manager_->queuesEmpty();
        const bool fps_empty = !fp_manager_ || fp_manager_->queuesEmpty();
        const bool out_empty = output_queue_.empty();

        if (lbs_empty && fps_empty && out_empty) {
            break;
        }

        std::this_thread::sleep_for(std::chrono::milliseconds(50));
    }
    std::cout << "[DPIEngine] Pipeline drained, shutting down.\n";
    ...
    // Print JSON summary (Added per requirement)
    std::cout << generateJsonSummary();
...
PacketJob DPIEngine::createPacketJob(const PacketAnalyzer::RawPacket& raw,
    ...
    if (job.data.size() > 14) {
        ...
        } else if (parsed.has_udp && (job.transport_offset + 8) <= job.data.size()) {
...
std::string DPIEngine::generateReport() const {
    ...
    ss << "  forwarded_packets=" << stats_.forwarded_packets.load() << "\n";
    ss << "  dropped_packets=" << stats_.dropped_packets.load() << "\n";

    if (stats_.total_packets > 0) {
        double drop_rate = 100.0 * stats_.dropped_packets.load() / stats_.total_packets.load();
        ss << "  drop_rate_pct=" << std::fixed << std::setprecision(2) << drop_rate << "\n";
    }

    if (lb_manager_) {
        auto lb_stats = lb_manager_->getAggregatedStats();
        ss << "  lb_received=" << lb_stats.total_received << "\n";
        ss << "  lb_dispatched=" << lb_stats.total_dispatched << "\n";
    }

    if (fp_manager_) {
        auto fp_stats = fp_manager_->getAggregatedStats();
        ss << "  fp_processed=" << fp_stats.total_processed << "\n";
        ss << "  fp_forwarded=" << fp_stats.total_forwarded << "\n";
        ss << "  fp_dropped=" << fp_stats.total_dropped << "\n";
        ss << "  active_connections=" << fp_stats.total_connections << "\n";
    }

    if (rule_manager_) {
        auto rule_stats = rule_manager_->getStats();
        ss << "  blocked_ips=" << rule_stats.blocked_ips << "\n";
        ss << "  blocked_apps=" << rule_stats.blocked_apps << "\n";
        ss << "  blocked_domains=" << rule_stats.blocked_domains << "\n";
        ss << "  blocked_ports=" << rule_stats.blocked_ports << "\n";
    }
...
// Added Implementation for JSON Summary
std::string DPIEngine::generateJsonSummary() const {
    std::ostringstream oss;
    
    // Retrieve stats from global connection table
    auto conn_stats = global_conn_table_->getGlobalStats();
    
    // Tally up the connections directly from the distribution map
    int total_conns = 0;
    int unknown_conns = 0;
    for (const auto& [app, count] : conn_stats.app_distribution) {
        total_conns += count;
        if (app == AppType::UNKNOWN) {
            unknown_conns += count;
        }
    }
    int classified_conns = total_conns - unknown_conns;
    
    oss << "{\n";
    oss << "  \"total_packets\": " << stats_.total_packets.load() << ",\n";
    oss << "  \"total_connections\": " << total_conns << ",\n";
    oss << "  \"classified\": " << classified_conns << ",\n";
    oss << "  \"unknown\": " << unknown_conns << ",\n";
    
    // Application Distribution
    oss << "  \"application_distribution\": {";
    bool first_app = true;
    for (const auto& [app, count] : conn_stats.app_distribution) {
        if (app == AppType::UNKNOWN) continue; 
        if (!first_app) oss << ",";
        oss << "\n    \"" << appTypeToString(app) << "\": " << count;
        first_app = false;
    }
    oss << "\n  },\n";
    ...
    for (const auto& [domain, count] : conn_stats.top_domains) {
        if (!first_domain) oss << ",";
        oss << "\n    \"" << domain << "\": " << count;
        first_domain = false;
    }
